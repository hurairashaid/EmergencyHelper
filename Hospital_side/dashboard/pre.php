...
$id = $_SESSION['id'];
$sql = "SELECT * FROM Patients WHERE hospital_id = $id and date_released = '0001-01-01' and pre = '1' and post = '0'";
$result = mysqli_query($conn , $sql);
$deleteid = $_GET['id'];
if (is_null($deleteid) == 0) {
date_default_timezone_set('Pakistan/Karachi');
$date = date('Y/m/d');
$sql1 = "UPDATE `Patients` SET `post` = '1' WHERE `Patients`.`id` = $deleteid;";
...
<table>
  ...
  <?php while($row = mysqli_fetch_array($result)):;?>
    <tr>
      ...
      <td data-column="date"><a href="http://localhost/Hospital_side/dashboard/pre.php?id=<?php echo $row['id'] ?>">Add</a></td>
    </tr>
  <?php endwhile;?>
